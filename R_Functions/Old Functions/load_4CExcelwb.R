#load_4CExcelwb Function

load_4CExcelwb <- function(filename, directory = "Data"){ 
        ...
        j<-0 # Loop counter
        for(i in namelist){ #repeat loop for each sheet in Excel
                j<- j+1 
        ...
        IDek <- as.character((assign.eklab[,1]))        #List of student IDs for Ek lab (type:character)
        ek.4Cdf <- data.frame(IDcode=IDek) #Empty dataframe for 4C scores with ID in first column
        ek.converted <- digit_extraction(ek.4Cdf, assign.eklab) ##converts 6 digit number to seperate columns of data frame
        cccclist <- list() #list to store 4c matrices (will be added to dataframe)
        
        j <- 0
        for(i in IDek){
                j <- j+1
                tempframe <- subset.data.frame(ek.converted, ek.converted$IDcode==i) #subset single student 4C matrix
        ...
        tr.converted <- digit_extraction(tr.4Cdf, assign.trlab)
        cccclist <- list() #list to store 4c matrices (will be added to dataframe)
        
        j <- 0
        for(i in IDtr){
                j <- j+1
                tempframe <- subset.data.frame(tr.converted,tr.converted$IDcode==i) #subset single student 4C matrix
                ccccmatrix <- as.matrix(subset.data.frame(tempframe,select = comp:clarity))
                rownames(ccccmatrix) <- 1:(nrow(ccccmatrix)) #statement number as 4C matrix rowname
                cccclist[[j]] <- ccccmatrix
        }  
